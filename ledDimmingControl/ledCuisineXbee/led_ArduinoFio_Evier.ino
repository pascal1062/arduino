...
void loop()
{
   ...
   //Bytes venant de Xbee ex: 15 - 01 - 127
   //15 --> byte de welcome, 01 --> adresse de la led a dimmer, 127 --> taux de modulation (de 0 @ 255)
   //Ici adresses des led "1 et 2" installé dans le coin évier. 1 --> 1er set de led
   if ((data0 == 15) && (data1 == 1)) { //Data In Xbee pour 1er set de led (addr. 1)
     led = 0;
     button1.click = 0; 
   }
   if ((data0 == 15) && (data1 == 2)) { //Data In Xbee pour 2e set de led (addr. 2)
     led = 1;
     button2.click = 0; 
   }
   
   if ((data0 == 15 && (data1 == 1 || data1 == 2))) {
     if (data2 == 0) {  
       DimZero();
     } else if (fadeValue[led] < ((double)data2) - 5.0) {
        DimUp(5);  
     } else if (fadeValue[led] > ((double)data2) + 5.0) {
        DimDown(5);
     } else {
